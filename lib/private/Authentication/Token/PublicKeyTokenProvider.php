...
class PublicKeyTokenProvider implements IProvider {
	...
	public function generateToken(string $token,
		...
		$this->cache->set($token, serialize($dbToken), self::TOKEN_CACHE_TTL);
	 ...
	 */
	private function getTokenFromCache(string $tokenHash): ?OCPIToken {
		...
		return $token instanceof OCPIToken ? $token : null;
	}

	private function cacheToken(OCPIToken $token): void {
	...
	public function invalidateOldTokens() {
		...
		foreach ($this->mapper->listOld($olderThan, OCPIToken::DO_NOT_REMEMBER) as $tokenHash) {
			$this->mapper->invalidate($tokenHash);
			$this->cacheInvalidHash($tokenHash);
		}
		...
		foreach ($this->mapper->listOld($rememberThreshold, OCPIToken::REMEMBER) as $tokenHash) {
			$this->mapper->invalidate($tokenHash);
			$this->cacheInvalidHash($tokenHash);
		}
	}
	...
	public function invalidateLastUsedBefore(string $uid, int $before): void {
		foreach ($this->mapper->listLastUsedBefore($uid, $before) as $tokenHash) {
			$this->mapper->invalidate($tokenHash);
			$this->cacheInvalidHash($tokenHash);
		}
	}
